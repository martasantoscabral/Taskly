...
export default function DesafioPage() {
  ...
  async function aderirDesafio(id) {
    try {
      ...
      if (!res.ok) {
        setErro(
          data.error ||
          "Não foi possível participar no desafio.",
        );
        return;
      }
  ...
  const desafiosPublicados = desafios.filter(
    (desafio) =>
      desafio.publicado &&
      !desafio.concluido,
  );

  const desafiosParticipados = desafios.filter(
    (desafio) =>
      desafio.participacoes?.some(
        (participacao) =>
          participacao.id_user === meuId,
      ),
  );

  const desafiosHistorico = desafios.filter(
    (desafio) =>
      desafio.participacoes?.some(
        (participacao) =>
          participacao.id_user === meuId &&
          participacao.concluido,
      ),
  );

  const desafiosAtivos = desafiosPublicados.filter(
    (desafio) =>
      !desafio.participacoes?.some(
        (participacao) =>
          participacao.id_user === meuId &&
      ...
      <div className="pagina-user">
        ...
        <section className="cabecalho-tarefas">
          <div>
            ...
            <p>
              Participa em desafios e acompanha o teu
              progresso.
            </p>
          </div>
        </section>
      ...
      <main>
        ...
        {aCarregar ? (
          <div className="estado-vazio-desafios">
            <h3>A carregar desafios...</h3>
          </div>
        ) : (
          <>
            <div className="grelha-estatisticas-desafios">
              <div className="cartao-estatistica-desafio">
                <div className="rotulo-estatistica-desafio">
                  ATIVOS
                </div>

                <div className="valor-estatistica-desafio">
                  {desafiosAtivos.length}
                </div>

                <div className="texto-estatistica-desafio">
                  disponíveis
                </div>
              </div>
            ...
            {desafioAtual ? (
              <section className="desafio-destaque-user">
                <h2 className="titulo-desafio-destaque">
                  {desafioAtual.titulo}
                </h2>

                <p className="descricao-desafio-destaque">
                  Badge: {desafioAtual.badge}
                </p>

                <div className="etiquetas-desafio">
                  <span className="etiqueta-desafio">
                    {desafioAtual.participacoes?.length ||
                      0}{" "}
                    participantes
                  </span>
                  ...
                  <span className="etiqueta-desafio">
                    {diasRestantes(
                      desafioAtual.data_fim,
                    )}{" "}
                    dias restantes
                  </span>

                  <span className="etiqueta-desafio">
                    Medalha: {desafioAtual.badge}
                  </span>
                </div>
                  ...
                  {desafiosAtivos.map((desafio) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="descricao-item-desafio">
                          Badge: {desafio.badge}
                        </p>
                      </div>
              ...
              <section className="cartao-lista-desafios">
                ...
                {desafiosHistorico.length === 0 ? (
                  <div className="historico-vazio">
                    <p>
                      Ainda não concluíste desafios.
                    </p>
                  </div>
                ) : (
                  <div className="lista-historico-desafios">
                    {desafiosHistorico.map(
                      (desafio) => (
                        <div
                          key={desafio.id_desafio}
                          className="item-historico-desafio"
                        >
                          <div>
